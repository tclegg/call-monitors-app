...
function post(row) {
	...
	return new Promise(function (resolve, reject) {
		//console.log(db)
		db.monitors.insert(row, function (err, result) {
			if (err) {
				reject(err)
			} else {
				resolve(result)
...
var ClaimedMonitors = {
	pull: function () {
		...
		return new Promise(function (resolve, reject){
			db.claimedDb.find(query).sort({'agent': 1}).exec(function (err, result){
				if (err) {
					reject (err)
				} else {
					resolve (result)
	...
	post: function (leadAbbv, agentAbbv){
		...
		return new Promise(function (resolve, reject){
			db.claimedDb.insert(query, function(err, result) {
	...
	remove: function (agentAbbv) {
		...
		return new Promise(function (resolve, reject){
			db.claimedDb.remove(query, {}, function (err, result){
				if (err) {
					reject (err)
				} else {
					resolve (result)
...
function pullAgentMonitors(query, sort) {
	...
	return new Promise(function (resolve, reject) {
		//console.log(db)
		db.monitors.find(query).sort(sort).exec(function (err, result) {
			if (err) {
				reject(err)
			} else {
				resolve(result)
...
function dbAgentUpdate(args, field) {
	...
	return new Promise(function (resolve, reject) {
		switch (args.type) {
			...
			db.agentsDb.update({_id: args['edit-agent-modal-id'],}, {
					$set: {
			...
			console.log(args)
				if (args['remove-agent-modal-inactive'] == 1) {
					db.agentsDb.update({_id: args['remove-agent-modal-id']}, {
						'$set': {
							inactive: 0
						}
					}, {}, function (error, numReplaced) {
	...
	return new Promise(function (resolve, reject) {
		switch (args.type) {
			case 'edit-lead-modal':
				db.leadsDb.update({_id: args['edit-lead-modal-id']}, {
					$set: {
			...
			case 'remove-lead-modal':
				if (args['remove-lead-modal-inactive'] == 0) {
					db.leadsDb.update({_id: args['remove-lead-modal-id']}, {
						'$set': {
							inactive: 1
						}
					}, {}, function (error, numReplaced) {
...
function dbAddAgent(args, field) {
	...
	return new Promise(function (resolve, reject) {
		db.agentsDb.insert(inserted, function (error, insertedRow) {
			//done
			if (error) {
				//throw {message: error, field: $(field)}
				return reject({
...
function dbAddLead(args, field) {
	...
	return new Promise(function (resolve, reject) {
		db.leadsDb.insert(inserted, function (error, insertedRow) {
			//done
			if (error) {
				//throw {message: error, field: $(field)}
				return reject({
